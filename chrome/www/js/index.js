...
function buildEntryString(name, entry, image) {
	...
	return '<li data-timestamp="' + new Date(entry.publishedDate).getTime() + '" data-entry-id="' + entry.link + '">' 
	  ...
	  + '<p><a href="' + entry.link + '" target="_blank">' + dateArr[0] + ' ' + dateArr[2] + ' ' + dateArr[1] + ', ' + dateArr[3] + '</a></p>'	  
	  + '<h1>' + title + '</h1>'
	  ...
	  + '<div class="readLink"><a href="' + entry.link + '" target="_blank">Read</a></div>'	  
	  + '</div>'
	  + '</li>';
}
...
function fixContent(content) {
	...
	*/
	...
	if (content.match(linkRegex)) {
	    ...
	    content = content.replace(linkRegex, "$1" + '"' + "$2" + '" target="_blank"' + "$3");	    
	}
